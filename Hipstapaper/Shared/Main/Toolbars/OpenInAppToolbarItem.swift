...
func OpenInAppToolbarItem(_ features: WindowManager.Features,
                          shouldBulkOpen: @escaping (Bool) -> Void)
                          -> ToolbarItem<String, ButtonToolbar>
{
    ToolbarItem(id: "Detail.0") {
        ButtonToolbar(systemName: "safari",
                      accessibilityLabel: Verb.Open)
        {
            shouldBulkOpen(features.contains(.bulkActivation))
        }
//        .keyboardShortcut("o")
